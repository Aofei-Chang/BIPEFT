...
def calculate_zeta_for_shifting(weights, max_num_param, ranks, last_zeta=0, fix_mask=None, budget_high=0, epoch=None, all_epochs=120):
    ...
    param_nums_ffn = torch.tensor([rank * (768+3072) for rank in ranks], dtype=torch.float32)
    params_for_cal_norm = param_nums.unsqueeze(0).unsqueeze(0).expand_as(weights[:, :4]).cuda()
    params_for_cal_ffn = param_nums_ffn.unsqueeze(0).unsqueeze(0).expand_as(weights[:, 4:]).cuda()
    params_for_cal = torch.cat((params_for_cal_norm,params_for_cal_ffn), dim=1)
