...
class AbstractTask(abc.ABC):
    ...
    small_datasets_without_all_splits = ["cola", "wnli", "rte", "superglue-cb", "superglue-copa", "superglue-multirc",
                                         ...
                                         "superglue-boolq","web_nlg"]
    large_data_without_all_splits = ["qqp", "qnli", "superglue-record", "sst2","mnli"]
    ...
    def get(self, split, add_prefix=True, n_obs=None, split_validation_test=False):
        ...
        # set as the test set.
            ...
            indices = self.get_split_indices(split, dataset, validation_size=2000)
            dataset = self.subsample(dataset, n_obs, indices)
        else:
...
class SuperGLUECB(AbstractTask):
    ...
    split_to_data_split = {"train": "train",
                           "validation": "validation",
                           "test": "validation"}
    metric = [metrics.mean_multiclass_f1(num_classes=3,zero_division=1), metrics.accuracy]
...
class WebNLG_(AbstractTask):
    name = "web_nlg"
    # labels_list = ['0', '1']
    split_to_data_split = {"train": "train",
                           "validation": "dev",
                           "test": "test"}
    metric = [metrics.bleu_nltk]
    metric_names = ["bleu_nltk"] 
    # metric = [metrics.bleu,metrics.meteor,metrics.chrf]
    # metric_names = ["bleu","meteor","chrf"] 
    total_num = 0

    def load_dataset(self, split):
        return datasets.load_from_disk(r"/home/zhenzhang/dir2/s3delta/data/web_nlg_2020")[split]
    ...
    def preprocessor(self, example, add_prefix=True):
        ...
        seen = ['Airport','Astronaut','Building','City','ComicsCharacter',
            ...
            'Politician']
        length = len(example['modified_triple_sets'])
        for i in range(length):
            sents = example['lex'][i]['text']
            comments = example['lex'][i]['comment']
            triples = example['modified_triple_sets'][i]['mtriple_set'][0]
            cate = example['category'][i]
        
            temp_triples = ''
            if add_prefix:
                temp_triples += self.name+' '
            for triple in triples:
                temp_triples += f'({triple})'

            for s,sent in enumerate(sents):
                if comments[s] == 'good' or comments[s] == '':
                    full_tgt_lst.append(sent)
                    full_src_lst.append(temp_triples)
                    full_cate_lst.append(cate in seen)
                    full_idx_lst.append(self.total_num)
            self.total_num+=1
        x = {}
        x['source'] =  full_src_lst
        x['target'] = full_tgt_lst
        x['extra_fields'] = [{'if_seen':cate,'idx':idx} for cate,idx in zip(full_cate_lst,full_idx_lst)]
        x['task'] = [self.name for _ in range(len(full_src_lst))]
        return x
        
    def map_dataset(self, dataset, add_prefix):    
        new =  dataset.map(functools.partial(self.preprocessor, add_prefix=add_prefix),
                           batched=True,remove_columns=dataset.column_names)
