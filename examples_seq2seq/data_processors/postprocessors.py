...
class Record(PostProcessor):
    def process(self, preds, labels, data_info):
        ...
        return preds, labels 
class WebNLG(PostProcessor):
    def process(self, preds, labels, data_info):
        preds, labels = super().process(preds, labels, data_info)
        # print(preds, labels )
        outputs = {}
        for pred,label,info in zip(preds, labels, data_info):
            pair = outputs.get(info['idx'],{'source':None,'target':[],'if_seen':None})
            outputs[info['idx']] = pair
            if pair['source'] is None:
                pair['source'] = pred
                pair['if_seen'] = info['if_seen']
            pair['target'].append(label)
            # print(outputs)
        preds = []
        labels = []
        for pair in outputs.values():
            preds.append((pair['source'],pair['if_seen']))
            labels.append(pair['target'])
        return preds, labels 
