...
class LoRA_PEFT(nn.Module):
    def __init__(self, original_linear, LoRA_a:nn.Linear, LoRA_b:nn.Linear, LoRA_dim=8, candidate_dims=[1, 4, 8]):
        super().__init__()
        ...
        self.super_LoRA_dim = LoRA_dim
        self.candidate_dims = candidate_dims
        self.candidate_dims_binary = [0, 8]
    ...
    def forward(self, x, gumbel_weights=None, dimension_mask=None, iterative_order=None, main_forward=None):
        ...
        if gumbel_weights is not None:
            ...
            w_a_sampled, w_b_sampled = (
                self.sample_lora(self.LoRA_a.weight, self.LoRA_b.weight, gumbel_weights=gumbel_weights, dimension_mask=dimension_mask))
            x = self.original_linear(x) + F.linear(input=F.linear(x, weight=w_a_sampled), weight=w_b_sampled)
            # t2 = time.time()
            # print("lora sample time", t2 - t1)
        else:
            # t1 = time.time()
            x = self.original_linear(x) + self.LoRA_b(self.LoRA_a(x))
            # t2 = time.time()
            # print("lora original time", t2 - t1)
        return x
