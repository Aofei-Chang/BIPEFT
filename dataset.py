...
class ImageFilelist(data.Dataset):
    def __init__(self, root, flist, transform=None, target_transform=None,
                 flist_reader=default_flist_reader, loader=default_loader, quick_eval=False):
        self.root = root
        if quick_eval:
            self.imlist = flist_reader(flist)[:400]
        else:
...
def get_data(name, evaluate=True, batch_size=64,few_shot=False,shot=1,seed=1,mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    if few_shot:
        ...
        root='/data/aofei/vit/vtab-1k/' + name
        root_train = root
        root_val = root
    ...
    if evaluate:
        ...
        test_loader = torch.utils.data.DataLoader(
            ImageFilelist(root=root_val, flist=test_flist,
                transform=val_transform, quick_eval=True),
            batch_size=256, shuffle=False,
            num_workers=4, pin_memory=True)
    else:
        train_loader = torch.utils.data.DataLoader(
        ...
        test_loader = torch.utils.data.DataLoader(
            ImageFilelist(root=root_val, flist=test_flist,
                          transform=val_transform, quick_eval=False),
            batch_size=256, shuffle=False,
            num_workers=4, pin_memory=True)
