...
do
  ...
  do
      ...
      output_dir="/data/user_name/output/NLP/mom/search/${DATASET}/${seed}/"
      log_file="/data/user_name/output/NLP/mom/search/${DATASET}_seed${seed}.log"
      CUDA_VISIBLE_DEVICES=$cuda_device torchrun --nproc_per_node 1  --master_port $master_port train.py \
          --model_name_or_path google-t5/t5-large \
          --task_name="${DATASET}" \
          --log_dir=/data/user_name/output/NLP/mom/search/logs/ \
          --train_batch_size 64 \
          --valid_batch_size 32 \
          --accum_iter 1 \
          --epochs $epoch \
...
do
  ...
  do
      ...
      cuda_device=${cuda_devices[$index]}
      master_port=${master_ports[$index]}
      resume="/data/user_name/output/NLP/mom/search/${DATASET}/${seed}/checkpoint.pth"
      output_dir="/data/user_name/output/NLP/mom/retrain/${DATASET}/${seed}/"
      log_file="/data/user_name/output/NLP/mom/retrain/${DATASET}_seed${seed}.log"
      CUDA_VISIBLE_DEVICES=$cuda_device torchrun --nproc_per_node 1  --master_port $master_port train.py \
          --model_name_or_path google-t5/t5-large \
          --task_name="${DATASET}" \
          --log_dir=/data/user_name/output/NLP/mom/search/logs/ \
