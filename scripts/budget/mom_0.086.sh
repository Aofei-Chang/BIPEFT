...
do
  ...
  do
      ...
      output_dir="/data/user_name/output/NLP/budget/0.086/search/${DATASET}/${budget}_${seed}/"
      log_file="/data/user_name/output/NLP/budget/0.086/search/${budget}_${DATASET}_seed${seed}.log"
      CUDA_VISIBLE_DEVICES=$cuda_device torchrun --nproc_per_node 1  --master_port $master_port train.py \
          --model_name_or_path google-t5/t5-large \
          --task_name="${DATASET}" \
          --log_dir=/data/user_name/output/NLP/budget/0.086/search/logs/ \
...
do
  ...
  do
      ...
      cuda_device=${cuda_devices[$index]}
      master_port=${master_ports[$index]}
      resume="/data/user_name/output/NLP/budget/0.086/search/${DATASET}/${budget}_${seed}/checkpoint.pth"
      output_dir="/data/user_name/output/NLP/budget/0.086/retrain/${DATASET}/${budget}_${seed}/"
      log_file="/data/user_name/output/NLP/budget/0.086/retrain/${budget}_${DATASET}_seed${seed}.log"
      CUDA_VISIBLE_DEVICES=$cuda_device torchrun --nproc_per_node 1  --master_port $master_port train.py \
          --model_name_or_path google-t5/t5-large \
          --task_name="${DATASET}" \
          --log_dir=/data/user_name/output/NLP/budget/0.086/retrain/logs/ \
          --train_batch_size 32 \
          --valid_batch_size 32 \
          --accum_iter 1 \
          --epochs $epoch \
